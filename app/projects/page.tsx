...
export default function ProjectsPage() {
  return (
    <div className="min-h-screen bg-[var(--color-page-bg)] flex flex-col font-mono text-[var(--color-text-primary)]">
      ...
      {/* Header/Back Row */}
      <div className="grid-row-container">
        <div className="grid-col-container bg-[var(--color-background)] px-5 py-4 flex items-center justify-between">
          <Link 
            href="/" 
            className="flex items-center gap-2 text-xs text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)] transition-colors group cursor-pointer"
          >
            <ArrowLeft className="w-3.5 h-3.5 transition-transform group-hover:-translate-x-1" />
            <span>Go Back Home</span>
          </Link>
          <span className="text-xs font-semibold uppercase tracking-wider text-[var(--color-text-muted)]">
            Archive // {projects.length} Items
          </span>
        </div>
      </div>
      ...
      {/* Main Title Row */}
      <div className="grid-row-container">
        <div className="grid-col-container bg-[var(--color-background)] px-5 py-10">
      ...
      {/* Projects Catalog Grid Row */}
      <div className="grid-row-container flex-grow">
        <div className="grid-col-container bg-[var(--color-background)] relative grid grid-cols-1 sm:grid-cols-2 divide-y sm:divide-y-0 divide-dashed divide-[var(--color-border)]">
          {projects.map((project, i) => {
            const isLeftColumn = i % 2 === 0;
            const isLastRow = i >= Math.floor((projects.length - 1) / 2) * 2;
            ...
            return (
              ...
              >
                ...
                <div className={`group bg-transparent overflow-hidden transition-all duration-300 hover:bg-[var(--color-surface)] h-full flex flex-col ${
                  !isLastRow ? "sm:border-b sm:border-dashed sm:border-[var(--color-border)]" : ""
                } ${
                  !isLastMobile ? "max-sm:border-b max-sm:border-dashed max-sm:border-[var(--color-border)]" : ""
                } ${
                  isLeftColumn ? "sm:border-r sm:border-dashed sm:border-[var(--color-border)]" : ""
                } cursor-default`}>

                  {/* Image area */}
                  <div className="relative aspect-[16/10] bg-[#080808] drafting-grid overflow-hidden border-b border-dashed border-[var(--color-border)] group-hover:bg-[#0c0c0c] transition-colors duration-300">
      ...
      {/* Footer Row */}
      <div className="grid-row-container">
        <div className="grid-col-container bg-[var(--color-background)] px-5 py-5 flex items-center justify-between text-xs text-[var(--color-text-muted)] font-mono">
          <p>Designed and Developed by Pratyush.</p>
